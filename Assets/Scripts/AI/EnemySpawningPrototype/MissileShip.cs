...
public class MissileShip : IEnemy
{
    ...
    void Start()
    {
        ...
        MyWeapon = gameObject.AddComponent<Rocket>();
        MyWeapon.Initialize(this, gameObject, MyPlanet.Planet);

        MyWeapon.projectileSpeed = 10;
        MyWeapon.ammo = int.MaxValue;
        MyWeapon.player = PlayerManager.players[0];
            ...
            if (path.Count > 0)
            {
                ...
                while (num < path.Count)
                {
                    ...
                    if (dist < activationDistance)
                    {
                        ...
                        if (elapsedTime > secondsBetweenShots)
                        {
                            StartCoroutine(ShootSequence());
                    ...
                    if (true == shouldWait)
                    {
                        elapsedTimeTimeout += Time.deltaTime;

                        //Wait to reach the next node in the path
                        if ((AdjustedMagnitudeFromCell(path[num - 1]) < MIN_CELL_DIST) || Time.deltaTime > 3f)
                        {
                            shouldWait = false;
                            elapsedTimeTimeout = 0f;
                        }
